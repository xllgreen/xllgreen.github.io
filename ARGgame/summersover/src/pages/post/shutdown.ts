 ...
 * 关站公告正文页 — 页面⑧
 * 夜航论坛 · 塔台版块
 * "因个人原因，夜航将于2016年10月1日零时起停止运营"
 *
 * 功能：搜索栏交互、路径追踪、告别帖入口
 * 与解谜链对齐：本页面为序号 ⑧，右下角显示 8/30。
 */
...
function init(): void {
  addPathLog('进入关站公告: 夜航即将停运')
  initSearch()

  // 告别帖入口
  const farewellLink = document.getElementById('farewell-link')
  if (farewellLink) {
    farewellLink.addEventListener('click', (e) => {
      e.preventDefault()
      addPathLog('关站公告 → 前往告别帖')
      window.open('/farewell/', '_blank')
    })
  }
