...
window.__end=id=>{if(!endingText[id])return;state.ending=id;state.stage=STAGE.END;safeSave();if(!meta.endingsSeen.includes(id))meta.endingsSeen.push(id);meta.completed=true;saveMeta();updateTitleButtons();const e=endingText[id];openModal(`<div class="ending"><p class="ending-code">${e.code}</p><p class="ending-title">${e.title}</p><p>${e.body}</p><div class="divider"></div><p>已解锁结局：${meta.endingsSeen.length}/4。二周目可以从“快速复盘”进入中段，不必重复前半段。</p><div class="choice-grid two"><button onclick="window.__showArchive()">查看结局档案</button><button onclick="window.__restart()">回到标题</button></div><div class="divider"></div></div>`,false)};
